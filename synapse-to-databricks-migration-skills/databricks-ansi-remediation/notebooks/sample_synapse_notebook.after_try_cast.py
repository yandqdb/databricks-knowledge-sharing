...
clean = (
    df.withColumn("zone_id", col("zone_raw").try_cast("int"))
      .withColumn("fare", expr("try_cast(fare_text as decimal(10,2))"))
)
...
# MAGIC SELECT
# MAGIC   route_id,
# MAGIC   TRY_CAST(card_serial AS BIGINT)                      AS card_id,
...
spark.sql("""
  ...
  SELECT route_id, TRY_CAST(tap_count AS SMALLINT) FROM tmp_counts
""")
